...
This repo has one research skill family (5 skills total). Each can be used standalone, but they're designed to compose. This document describes the canonical playbooks for cross-skill workflows.
...
| `sg-financial-advisor` | Singapore protection + portfolio review | User-provided facts + current rules | Markdown assessment | **Read** regulatory sources, **write** action checklist |
| `news-rss-watch` | Deterministic feed polling | T+0 / since last watermark | JSON or Markdown digest | **Read** RSS/Atom feeds, **write** watermark state |
