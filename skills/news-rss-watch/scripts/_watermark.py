"""Shared watermark helper for watcher scripts.

A watermark is a JSON file recording IDs seen on previous runs, so the
next run emits only items we haven't seen before.
...
- Atomic: write to .tmp then rename, so a crashed script can't leave a
  half-written state file that permanently breaks dedup.

State dir: $NEWS_WATCHER_STATE_DIR, else ~/.finance-skills/watcher-state/.
"""
...
def _state_dir() -> Path:
    override = os.environ.get("NEWS_WATCHER_STATE_DIR")
    if override:
        return Path(override)
    return Path.home() / ".finance-skills" / "watcher-state"
...
def format_items_as_markdown(
    ...
    *,
    ...
    """Render items as Markdown for delivery. Empty string when empty."""
    if not items:
        return ""
    lines: List[str] = []
    for item in items:
